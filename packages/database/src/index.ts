 ...
 * email (records are keyed by uuid, seeded from KNOWN_USERS) and only writes
 * when the URL actually changed, to avoid a write on every auth refresh.
 * Best-effort: failures (e.g. an unseeded user, or write rules) are swallowed.
 */

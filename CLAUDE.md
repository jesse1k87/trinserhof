...
- **`apps/server`** — Express API, deployed on Vercel (`vercel.json` rewrites everything to `apps/server/src`). Exposes `POST /submit` and `POST /update` (`apps/server/src/firebase.ts`). **Currently unused by client/form** — both write to Firebase directly, so these endpoints exist for a future integration (e.g. Stripe, which is referenced but not wired up). It does **not** use `firebase-admin` — `apps/server/src/firebase.ts` uses the regular `firebase/app` + `firebase/database` client SDK, lazily initialized with `@bookings/constants`'s `FIREBASE_CONFIG`, whose `databaseURL` is read via `dotenv` at **runtime** (not esbuild `define` at build time), so the same built bundle can point at different databases depending on the env vars present when it runs.
...
- **`@bookings/constants`** — `FIREBASE_CONFIG`: `apiKey`/`appId`/`authDomain`/`messagingSenderId`/`projectId`/`storageBucket` are hardcoded literals (non-secret, safe to commit); `databaseURL` is read from `process.env.FIREBASE_DATABASE_URL` since it differs between staging and production.

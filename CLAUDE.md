...
- **`@trinserhof/constants`** — `FIREBASE_CONFIG`, reading the 7 `FIREBASE_*` env vars from `process.env` (non-secret structure, safe to commit; the actual values come from env, never hardcoded here).

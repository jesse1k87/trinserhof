...
**Holidays:** `packages/supabase/scripts/fetch-holidays.ts` (run via `npm run fetch-holidays -w @trinserhof/supabase`) syncs public & school holidays from the free, no-auth [OpenHolidays API](https://www.openholidaysapi.org) into the `Region` and `Holiday` tables. Each country gets a synthetic country-level `Region` (id = country code, e.g. `AT`) that owns its nationwide holidays, plus one `Region` per subdivision (id = ISO 3166-2 code, e.g. `AT-7`); every source holiday becomes one `Holiday` row per region it applies to (nationwide → the country-level region; otherwise → each listed subdivision). The sync is idempotent: `Region` rows are upserted, and for each country the `Holiday` rows inside the requested date range are replaced so source corrections/removals propagate. Configurable via env vars or CLI flags (`--countries`, `--language`, `--from`/`--to` or `--years-ahead`, `--dry-run`); defaults to Austria (`AT`), German names, from Jan 1 this year through 2 more full years. Triggered manually by the **Fetch Holidays** GitHub workflow (`.github/workflows/fetch-holidays.yml`, `workflow_dispatch` with country/language/years-ahead/dry-run inputs), which runs `db:push` first so the tables exist. Node's built-in `fetch` needs direct outbound HTTPS (fine in CI/GitHub Actions).
...
- Nightly prices are keyed by room **type** (not by individual room). `RoomType.basePrice` is each type's default nightly price and is used **solely as a fallback**. The authoritative source is the `Price` table (`packages/supabase/prisma/schema.prisma`): each row has a `roomTypeId` (a real FK to `RoomType`), a `date`, a `base` amount, and a `markup` (unused by pricing resolution so far — reserved for a future signed adjustment on top of `base`), unique per `(roomTypeId, date)`. Types/schema live in `@trinserhof/types`'s `price.ts` (`Prices`, `EMPTY_PRICES`); writes go through `@trinserhof/supabase`'s `savePriceOverride` / `deletePriceOverride`.
- The PMS app reads prices via the `usePrices` hook, which fetches every `RoomType` (for its `basePrice` fallback) and every dated `Price` row, then shapes them into a `Prices` object (`base`: room type → fallback price from `RoomType.basePrice`; `overrides`: night → room type → the `Price` table's `base` for that night). There is currently no admin UI for editing per-night `Price` rows (the old **Prices** page was removed) — `RoomType.basePrice` is edited from the **Room types** page.
- `@trinserhof/helpers`'s `getStayPriceBreakdown` / `getRoomTypePriceForDate` / `getNightsInDateRange` are the central, reusable place a booking's price is resolved: for each night of a stay (check-in inclusive, check-out exclusive), a `Price`-table row for that room type + night wins; `RoomType.basePrice` is used only when the `Price` table has no row for that night. `PriceSummary` and `BookingFormFields` call them to compute/seed the booking's price for the selected room + date range. The total shown is display-only — only the per-night `pricePerNight` seeded from it is persisted on the booking.

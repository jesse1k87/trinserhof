...
const INPUT_FILE = resolve(rootDir, "data/backups_do_not_edit/raw_data_2026-06-14_firebase.json");
const OUTPUT_FILE = resolve(rootDir, "data/backups_do_not_edit/raw_data_2026-06-14_firebase_migrated.json");
const SUGGESTIONS_FILE = resolve(rootDir, "data/backups_do_not_edit/raw_data_2026-06-14_firebase_customer_suggestions.json");
...
try {
  ...
  const profiles = Object.values(dbData.customers).map(c => {
    ...
    const relatedBookings = Object.values(bookings).filter(b => b.customers?.includes(c.id));
  ...
  for (let i = 0; i < profiles.length; i++) {
    for (let j = i + 1; j < profiles.length; j++) {
      ...
      if (c1.normPhone && c2.normPhone) {
        ...
        } else if (c1.normPhone.length > 7 && c2.normPhone.length > 7 && 
  ...
  console.log(` - Merged into existing customers ${mergedCustomersCount} time(s).`);
  console.log(` - Found ${suggestions.length} potential customer duplicate pair(s).`);

  if (dryRun) {
    console.log(`\n[DRY RUN] Bypassed writing to ${OUTPUT_FILE}`);
    if (suggestions.length > 0) {
      console.log(`[DRY RUN] Bypassed writing suggestions to ${SUGGESTIONS_FILE}`);
    }
  } else {
